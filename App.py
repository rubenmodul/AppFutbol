...
from PySide6.QtCharts import QChart, QChartView, QBarSet, QBarCategoryAxis, QStackedBarSeries, QBarSeries
from PySide6.QtGui import QAction, QIcon, QPainter, QPixmap, Qt
...
class User(QMainWindow):
    def __init__(self):
        ...
        self.botoEstadistica = QPushButton("Estadistica")
        layoutBoto.addWidget(labelEstadistica)
        layoutBoto.addWidget(self.botoEstadistica)
        self.botoEstadistica.setDisabled(True)
        self.botoEstadistica.clicked.connect(self.checkInEstadistica)

        #Boto Mostrar Estadistiques
        self.botoMostrar = QPushButton("Mostrar Grafica")
        self.botoMostrar.setDisabled(True)
    ...
    def add_estadisticas(self):
        ...
        sql = f"SELECT * FROM jornadaJugador WHERE nomJugador = %s;"
        adr = [self.boxJuagdors.currentText()]
        ...
        # Recorremos la lista para añadir cada jornada
        for i in range(len(llistaEstadistica)):
            self.setgoles.append(llistaEstadistica[i][2])
            self.setminutos.append(llistaEstadistica[i][3])
            self.setactitud.append(llistaEstadistica[i][4])

        # Añadimos los parametros que tiene que tener el grafico
        estadisticas = QBarSeries()
    ...
    # Mostramos la finestra de estadisticasdel jugador
    ...
    def comprovarEquip(self, t):
        if t == "Prebenjamin A":
            ...
            for i in self.llistaJugadorsAlevin:
                self.boxJuagdors.addItem(i)
        # Desactivamos o activamos los botones dependiendo si hay seleccionado algun equipo
        if t != "":
            self.botoEstadistica.setEnabled(True)
            self.botoMostrar.setEnabled(True)
        else:
            self.botoEstadistica.setDisabled(True)
            self.botoMostrar.setDisabled(True)
            self.boxJuagdors.clear()
...
class Estadistica(QMainWindow):
    ...
    # Hacemos el insert para guardar los datos que ponemos en nuestra base de datos
    def guardar(self):
        self.hide()
        miConexion = mysql.connector.connect(host='localhost',
                                            user= 'root',
...
class About(QMainWindow):
    ...
    # Para volver a la ventana de User
    def tornarUser(self):
        self.hide()
